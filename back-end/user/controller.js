...
exports.user_create = [
  ...
  body("pin").trim().isLength(6).withMessage("pin length should be six digits"),
...
exports.user_login = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ error: "authentication failed incorrect email/password" });
  }
  ...
  const userAuth = { userId: user._id, role: user.role };

  const access_token = jwt.sign(userAuth, "your-secret-key", {
    expiresIn: 30 * 60,
  });

  const refresh_token = jwt.sign(userAuth, "your-secret-key", {
    expiresIn: "1d",
  });
